...
def main():
    ...
    if args.mode == '1':
        print(f"Running Mode 1 (HDBSCAN Clustering) with config={args.config}, output={args.output}")
        ...
        print(f"Running Mode 3 (Random Control HDBSCAN) with config={args.config}, output={args.output}")
        if args.min_cluster_size is not None:
            print(f"  Overriding min_cluster_size={args.min_cluster_size}")
        if args.min_samples is not None:
            print(f"  Overriding min_samples={args.min_samples}")
