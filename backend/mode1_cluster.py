...
def run_mode1(config_path="config.toml", output_dir="output", use_mass_filtering=True, 
    ...
    if use_mass_filtering:
        print(f"Using mass filtering with threshold {mass_outlier_threshold} dex")
        stable_haloes, positions, masses, halo_provenance, cluster_labels = enhanced_find_stable_haloes(
            mcmc_data, config, 
    ...
    for i, halo in enumerate(sorted_haloes[:10]):
        ...
        if 'log_mass_std' in halo:
            print(f"  Log-mass mean: {halo['mean_log_mass']:.2f} ± {halo['log_mass_std']:.2f}")
            print(f"  Log-mass range: {halo['log_mass_range']:.2f} dex")
